  ...
  try {
    ...
    const uproles = await Promise.all(
      company.roles.map(async (role) => {
        ...
        return {
          ...role._doc,
          InterviewScheduled: inProcess,
          AwaitingJoining: awaiting,
          OfferDrop: offerDrop,
          Joined: joined,
          Rejects: rejected,
        };
  ...
  try {
    ...
    const upcompanies = await Promise.all(
      companies.map(async (company) => {
        const allRemarks = await Remark.find({ companyId: company._id })
          .select("remarks")
          .sort({ createdAt: -1 })
          .lean()
          .exec();
        ...
        const combinedRemarks = allRemarks
          .map((item) => item.remarks)
          .join(", ");

        return {
          ...company,
          remarks: combinedRemarks || company.remarks || "",
          InterviewScheduled: inProcess,
          AwaitingJoining: awaiting,
          OfferDrop: offerDrop,
          Joined: joined,
...
const downloadRole = async (req, res) => {
  try {
    const { companyId, roleId } = req.params;
    const company = await Company.findOne({ _id: companyId });
    if (!company) throw new NotFoundError("Company not found with given id");
    ...
    // Sanitize and organize data using utility function
    const data = getRolePdfData(company, role);

    const doc = new PDFDocument({ margin: 50 });

    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${role.role || "role"}_${roleId}.pdf"`,
    );
    ...
    doc
      .fontSize(20)
      .font("Helvetica-Bold")
      .text("ROLE INFORMATION", { align: "center" });
    doc.moveDown();
    doc
      .fontSize(16)
      .font("Helvetica-Bold")
      .text(role.role || "N/A", { align: "center" });
    doc.moveDown(2);
    ...
    doc.moveDown(0.5);
    doc.fontSize(11).font("Helvetica").text(`Company: ${data.company.name}`);
    doc.text(`HR Name: ${data.company.hrName}`);
    doc.text(`HR Email: ${data.company.hrEmail}`);
    doc.text(`HR Mobile: ${data.company.hrMobile}`);
    doc.moveDown();
    ...
    doc
      .fontSize(14)
      .font("Helvetica-Bold")
      .text("Role Details", { underline: true });
    doc.moveDown(0.5);
    doc.fontSize(11).font("Helvetica");
    doc.text(`Designation: ${data.role.designation}`);
    doc.text(`Status: ${data.role.status}`);
    doc.text(`Process Type: ${data.role.processType}`);
    doc.text(`Job Type: ${data.role.period}`);
    doc.text(`Industry: ${data.role.industry}`);
    doc.text(`Experience: ${data.role.experience}`);
    doc.text(`Salary: ${data.role.salary}`);
    doc.text(`Shift: ${data.role.shift}`);
    doc.text(`Bond: ${data.role.bond}`);
    doc.text(`Age Criteria: ${data.role.ageCriteria}`);
    doc.text(`Cab Facility: ${data.role.cabFacility}`);
    doc.text(`Notice Period/Buyout: ${data.role.processWorkType}`);
    ...
    doc.moveDown(0.5);
    doc.fontSize(11).font("Helvetica");
    doc.text(`Locations: ${data.location.locations}`);
    doc.text(`Area: ${data.location.area}`);
    doc.moveDown();
    ...
    doc.moveDown(0.5);
    doc.fontSize(11).font("Helvetica");
    doc.text(`Mandatory Skills: ${data.skills.mandatory}`);
    doc.text(`Optional Skills: ${data.skills.optional}`);
    doc.moveDown();
    ...
    doc
      .fontSize(14)
      .font("Helvetica-Bold")
      .text("Qualifications", { underline: true });
    doc.moveDown(0.5);
    doc.fontSize(11).font("Helvetica");
    doc.text(`Qualifications: ${data.qualifications.list}`);
    doc.moveDown();
    ...
    doc.moveDown(0.5);
    doc.fontSize(11).font("Helvetica");
    doc.text(`Roles & Responsibilities: ${data.additional.happens}`);
    doc.moveDown();
    doc.text(`Other Docs: ${data.additional.otherDocs}`);
    doc.moveDown();
...
const getCompanyAndRoleNamesForCandidate = async (req, res) => {
  try {
    ...
    const companies = await Company.find(query)
      ...
      .populate("roles", " _id role billingTerm endTrackingDate")
      .exec();
...
const exportSelectedRolesExcel = async (req, res) => {
  try {
    ...
    const companies = await Company.find({ roles: { $in: ids } })
      .select("_id companyName roles")
      .lean();

    const companyNameByRole = new Map();
    companies.forEach((company) => {
      (company.roles || []).forEach((roleId) => {
        companyNameByRole.set(roleId.toString(), company.companyName || "");
      });
    });
    ...
    worksheet.columns = [
      ...
      { header: "Mandatory Skills", key: "mandatorySkills", width: 30 },
      { header: "Optional Skills", key: "optionalSkills", width: 30 },
      { header: "Qualification", key: "qualification", width: 25 },
      { header: "Shift", key: "shift", width: 12 },
      { header: "Salary", key: "salary", width: 15 },
      { header: "Cab Facility", key: "cabFacility", width: 12 },
      { header: "Location", key: "location", width: 25 },
      { header: "Area", key: "area", width: 20 },
      { header: "Bond", key: "bond", width: 10 },
      { header: "Age Criteria", key: "ageCriteria", width: 15 },
      { header: "Period", key: "period", width: 16 },
      { header: "Other Docs", key: "otherDocs", width: 20 },
      { header: "Original JD", key: "originalJD", width: 20 },
      { header: "FAQs", key: "faqs", width: 20 },
      { header: "Rejection Reasons", key: "rejectionReasons", width: 25 },
      { header: "Billing Term", key: "billingTerm", width: 12 },
      { header: "End Tracking Date", key: "endTrackingDate", width: 18 },
      { header: "Industry", key: "industry", width: 20 },
      { header: "About Company", key: "aboutCompany", width: 25 },
    ];

    roles.forEach((role) => {
      worksheet.addRow({
        roleId: role.roleId || role._id?.toString() || "",
        companyName: companyNameByRole.get(role._id?.toString()) || "",
        role: role.role || "",
        designation: role.designation || "",
        status: role.status ? "Active" : "Inactive",
        processType: role.processType || "",
        experience: role.experience || "",
        mandatorySkills: Array.isArray(role.mandatorySkills)
          ? role.mandatorySkills.join(", ")
          : "",
        optionalSkills: Array.isArray(role.optionalSkills)
          ? role.optionalSkills
              .map((group) => (Array.isArray(group) ? group.join(", ") : ""))
              .filter(Boolean)
              .join(" | ")
          : "",
        qualification: Array.isArray(role.qualification)
          ? role.qualification.join(", ")
          : "",
        shift: role.shift || "",
        salary: role.salary || "",
        cabFacility: role.cabFacility ? "Yes" : "No",
        location: Array.isArray(role.location) ? role.location.join(", ") : "",
        area: role.area || "",
        bond: role.bond ?? "",
        ageCriteria: role.ageCriteria || "",
        period: role.period || "",
        otherDocs: role.otherDocs || "",
        originalJD: role.originalJD || "",
        faqs: role.faqs || "",
        rejectionReasons: Array.isArray(role.rejectionReasons)
          ? role.rejectionReasons.join(", ")
          : "",
        billingTerm: role.billingTerm ?? "",
        endTrackingDate: role.endTrackingDate
          ? new Date(role.endTrackingDate).toLocaleDateString()
          : "",
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${name || "roles"}.xlsx"`,
    );
